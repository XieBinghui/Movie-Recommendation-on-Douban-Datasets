...
def load_data_kg():
	...
	movieFile = open("movie.csv","r",encoding='utf-8')
	...
	count = 0
	for item in reader:
		if item[3] not in entity.keys():
			entity[item[3]] = count #电影id
	...
	for item in reader:
		...
		if item[0] not in entity.keys():
			entity[item[0]] = count #评分
			count = count + 1
		rela_h.append(entity[item[3]])
		rela_r.append(relation['movie_movie_rate'])
		rela_t.append(entity[item[0]])
	...
	for item in reader:
		...
		for director in item[4][1:len(item[4])-1].split(","):
	...
	for item in reader:
		...
		#选取主演
		for actor in item[5][1:len(item[5])-1].split(",")[0:3]:
	...
	movieFile.close()
	#对电影时长信息进行处理
	relation['movie_movie_long'] = relation_count
	relation_count = relation_count + 1
	movieFile = open("movie.csv","r",encoding='utf-8')
	reader = csv.reader(movieFile)
	for item in reader:
		if item[7] == None: continue
		if item[7] not in entity.keys():
			entity[item[7]] = count #电影时长
			count = count + 1
		rela_h.append(entity[item[3]])
		rela_r.append(relation['movie_movie_long'])
		rela_t.append(entity[item[7]])
	...
	for item in reader:
		...
		for type_ in item[8][1:len(item[8])-1].split(","):
...
def load_data_rate(item_list,entity):
	...
	usrFile = open("user.csv","r",encoding='utf-8')
	...
	for u in reader:
		if u[7] not in usr:
			...
			user_pos_ratings[usr[u[7]]] = set()
			user_neg_ratings[usr[u[7]]] = set()
		for pair in u[4][1:len(u[4])-1].split(","):
			try:
				it,score =  pair.split(":")[0].strip().replace('\'',''),pair.split(":")[1].strip()
				if it in entity.keys():
					if score == 'None':
						if random.random() <= p:
							user_pos_ratings[usr[u[7]]].add(entity[it])
							rate.append((usr[u[7]],1,entity[it]))
						else:user_neg_ratings[usr[u[7]]].add(entity[it])
					elif int(score) >= 4:
						user_pos_ratings[usr[u[7]]].add(entity[it])
						rate.append((usr[u[7]],1,entity[it]))
					else :
						user_neg_ratings[usr[u[7]]].add(entity[it])
			except:
				continue
		unwatch_set = set(item_list) - user_pos_ratings[usr[u[7]]] -  user_neg_ratings[usr[u[7]]]
		for it in np.random.choice(list(unwatch_set), size=len( user_pos_ratings[usr[u[7]]] ), replace=False):
			rate.append((usr[u[7]],0,it))
	return usr,rate
