...
def parse_args():
    ...
    parser.add_argument('--keep_prob', type=float, default=1, # TODO
    ...
    parser.add_argument('--optimizer', nargs='?', default='AdagradOptimizer',
                        help='Specify an optimizer type (AdamOptimizer, rAdagradOptimizer, GradientDescentOptimizer, MomentumOptimizer).')
    ...
    parser.add_argument('--embedding_path', type=str, default="embedding/douban_deepwalk_original_64.npy")
    parser.add_argument('--neg_num', type=int, default=2,
                        help='Number of negative samples for each user in train data')
    parser.add_argument('--log_path', type=str, default='plus-1-06-08',
...
class FM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            self.squared_features_emb = tf.square(nonzero_embeddings) * tf.square(self.train_coeff_expend)
    ...
    def _initialize_weights(self):
        ...
        if self.pretrain_flag > 0:
            ...
            embedding_input = np.load(self.args.embedding_path)
            height, width = embedding_input.shape
            embedding = np.random.normal(0.0, 0.01, size=[self.features_M, self.hidden_factor])
            print("the shape of embedding")
            print(embedding.shape)
            embedding[:height] = embedding_input / 500
            embedding = embedding.astype(np.float32)
            print("the shape of new embedding")
            print(embedding.shape)
            print("the height and the width")
            print(height)
            print(width)
            # print(pp)
            all_weights['feature_embeddings'] = tf.convert_to_tensor(
                embedding,
    ...
    def train(self, Train_data_raw, Validation_data, Test_data):  # fit a dataset
        # Check Init performance
        Train_data = self.dataset.neg_sampling(Train_data_raw,  n=self.args.neg_num)
        del self.dataset.dict_neg
        gc.collect()
        ...
        for epoch in range(self.epoch):
            ...
            # for batch_xs in self.batch_gen(Train_data, self.batch_size, total_batch):
    ...
    def user_detect(self, features, y_true, y_score, offset_range=[0, 1000]):
...
if __name__ == '__main__':
    ...
    logging.info("==============")
    logging.info(args)
    data = DATA.LoadData(args.path, args.dataset, args.loss_type, args.feature_length)
    if args.verbose > 0:
    ...
    # Training
    t1 = time()
    model = FM(data.features_M, args.pretrain, save_file, args.hidden_factor, args.loss_type, args.epoch, args.batch_size, args.lr, args.lamda, args.keep_prob, args.optimizer, args.batch_norm, args.verbose, data, args)
    ...
    best_epoch = int(model.valid_rmse.index(best_valid_score))
