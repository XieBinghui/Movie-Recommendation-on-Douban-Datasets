...
Modified By Zhankui He for Recommender System for explicit feedback and multi-features
...
def parse_args():
    ...
    parser.add_argument('--log_path', type=str, default='douban-plus-1-06-08',
...
class NeuralFM(BaseEstimator, TransformerMixin):
    def __init__(self, features_M, hidden_factor, layers, loss_type, pretrain_flag, epoch, batch_size, learning_rate, lamda_bilinear,
                 keep_prob, optimizer_type, batch_norm, activation_function, verbose, early_stop, dataset, args, random_seed=2016):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            self.Feature_bias = tf.reduce_sum(tf.nn.embedding_lookup(self.weights['feature_bias'], self.train_features) * self.train_coeff_expend, 1)  # None * 1
    ...
    def evaluate(self, data):  # evaluate the results for an input set
        ...
        for b in range(total_batch):
            ...
            feed_dict = {self.train_features: self.padding(data['X'][begin: end]),
                         ...
                         self.dropout_keep: [1.0 for _ in range(len(self.layers))],
        ...
        if self.loss_type == 'square_loss':
            ...
            NDCG = self.ndcg_score(data['X'], y_true, y_pred, 10)
...
if __name__ == '__main__':
    ...
    logging.basicConfig(level=logging.INFO,
                        ...
                        filename=os.path.join(args.log_path, "NeuralFM%s.txt" % log_name))
    print(args)
    logging.info("==============")
    logging.info(args)
    ...
    # Training
    t1 = time()
    model = NeuralFM(data.features_M, args.hidden_factor, eval(args.layers), args.loss_type, args.pretrain, args.epoch, args.batch_size, args.lr, args.lamda, eval(args.keep_prob), args.optimizer, args.batch_norm, activation_function, args.verbose, args.early_stop, data, args)
