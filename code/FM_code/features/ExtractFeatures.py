...
def connectSql():
    engine = create_engine('mysql://root:bigdata10@localhost:3306/douban', convert_unicode=True, encoding='utf-8',
...
def pos_sample(df_movie, df_user, MovieId2Idx, UserIdx2following, following_rates, TypeDict, bias, path):
...
def neg_sample(df_movie, df_user, MovieId2Idx, UserIdx2following, following_rates, TypeDict, bias, path):
...
def split_dataset(path):
    ...
    for (num, line) in enumerate(pos_file.readlines()):
        ...
        elif num - before_line > 2 and ur == '999':    # 999 need change based on the number of users
...
if __name__ == '__main__':
    ...
    path = './data/douban'    # 40381 douban.pos, 959619 douban.neg
    print(usr_len)
    print(movie_len)

    # MovieId2Idx, UserIdx2following, following_rates, typeDict = preprocess(df_movie, df_user)
    # pos_sample(df_movie, df_user, MovieId2Idx, UserIdx2following, following_rates, typeDict, bias, path)
    # neg_sample(df_movie, df_user, MovieId2Idx, UserIdx2following, following_rates, typeDict, bias, path)
    # split_dataset(path)